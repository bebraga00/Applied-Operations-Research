    ...
    def pivot(self):
        ...
        for i in range(self.A.shape[0]):
            ...
            for j in range(self.A.shape[0]):
                if(j != i) and self.A[j][self.basis[i]] != 0:
                    coef = self.A[j][self.basis[i]]
                    self.A[j] -= coef * self.A[i]
                    self.b[j] -= self.b[i] * coef
                else:
                    continue
            # l4
            coef = self.c[self.basis[i]]
            self.c -= self.A[i] * coef
            self.bestObjective += coef * self.b[i]
        ...
        if(not(self.isMinimization)):
            compare = max(self.c)
            if(compare > 0 + epsilon):
                entrante = np.argmax(self.c) # variable entrante
            else:
                return False
        else:
            compare = min(self.c)
            if(compare < 0 - epsilon):
                entrante = np.argmin(self.c) # variable entrante
            else:
                return False
        ...
        ratios = np.zeros(len(self.b))
        for i in range(len(self.b)):
            if(self.A[i][entrante] == 0):
                ratios[i] = np.infty
            else:
                ratios[i] = self.b[i] / (self.A[i][entrante])

        for i in range(len(ratios)):
            if(ratios[i] < 0):
                ratios[i] = np.infty
                
        sortante = self.basis[np.argmin(ratios)]

        # 3 - Retourner vrai si une nouvelle base est trouvée et faux sinon
        self.basis[list(self.basis).index(sortante)] = entrante
